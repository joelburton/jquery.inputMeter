...
var meter=$('<meter></meter>',{min:0,max:opts.maxLength,low:0,high:opts.warnLength-1,css:css}).insertAfter(input);adjustProgress(0);input.keydown(handleKeystroke);function handleKeystroke(event){var key=event.which;var allowKey;var adjustment=1;console.log(key);if((key>=37&&key<=40)||key===9||key===18||key===16||key===27||event.ctrlKey||event.metaKey)
{return true;}
